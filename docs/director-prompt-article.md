...
ПРАВИЛА SLIDES:
- **РОВНО 4, 6 или 8 слайдов — ВСЕГДА ЧЁТНОЕ число** (Telegram-карусель красивее ложится на сетку 2×N). Никаких 5/7.
- Выбор количества по объёму контента: мало фактов → 4, средне → 6, много → 8.
...
**ANTI-EMPTY RULE (КРИТИЧНО — НАРУШЕНИЕ = БРАК)**:

Шаг 1 — посчитай факты в статье: числа/цифры со смыслом, ключевые утверждения, механики/шаги.

Шаг 2 — выбери TOTAL:
  - 4 фактов или меньше → TOTAL=4 (cover + big_number + bullet_list + outro_cta)
  - 5-8 фактов → TOTAL=6
  - 9+ фактов → TOTAL=8
Правило: если на полпути закончились факты — УМЕНЬШАЙ TOTAL, не дотягивай пустотой.

Шаг 3 — ЗАПРЕЩЕНО:
- bullet_list с менее чем 3 заполненными буллетами
- любой слайд где есть section_title но bullet_1, bullet_2, bullet_3 = ""
- quote_block где quote_text пуст или короче 60 chars
- compare_split где side_a_text или side_b_text пуст
- numbered_list где item_1_title пуст
- "пустые" слайды с одним заголовком и больше ничем

Шаг 4 — self-check: перед возвратом JSON, проверь каждый slide на минимум 3 непустых поля. Иначе удали + пересчитай page_counter, уменьши TOTAL до чётного.
